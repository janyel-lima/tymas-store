...
// Use Vue Router Route composition
const route = useRoute();

// Plan dictionary for metadata receipt display
...
onMounted(() => {
  ...
  if (parsedUserId) {
    ...
    userId.value = 'Usuário do Telegram';
  }

  const parsedPlano = getParam('plano') || getParam('plan');
  if (parsedPlano) {
    plano.value = parsedPlano;
  }

  const parsedBot = getParam('bot');
  if (parsedBot) {
    botUsername.value = parsedBot;
  }
});

// Compute dynamic bot link
const telegramBotUrl = computed(() => {
  if (!botUsername.value) {
    return 'https://t.me'; // Fallback redirect
  }
  // If it's already an absolute URL return as is
  if (botUsername.value.startsWith('http')) {
    return botUsername.value;
  }
  // Otherwise prefix with https://t.me/
  const cleanUsername = botUsername.value.replace('@', '');
...
<template>
  ...
  >
    ...
    <!-- Background visual decor (Aurora gradients in dark mode) -->
    <div v-if="isDark" class="absolute -top-40 left-1/2 -translate-x-1/2 w-96 h-96 bg-emerald-500/10 rounded-full blur-[100px] pointer-events-none"></div>
    <div v-if="isDark" class="absolute -bottom-40 left-1/2 -translate-x-1/2 w-96 h-96 bg-sky-500/5 rounded-full blur-[120px] pointer-events-none"></div>
    ...
    >
      ...
      <!-- Top aesthetic border lines -->
      <div class="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-emerald-500/20 via-emerald-400 to-sky-500/20"></div>
      ...
      <!-- Success Typography -->
      <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[11px] font-bold text-emerald-500 tracking-wider uppercase mb-3">
        <Sparkles class="h-3 w-3 shrink-0" />
        Pagamento Confirmado
      </span>

      <h1 :class="['text-2xl font-black tracking-tight mb-2', isDark ? 'text-white' : 'text-neutral-900']">Sucesso Confirmado!</h1>
      ...
      >
        <div :class="['flex items-center gap-2 mb-3 pb-3 border-b', isDark ? 'border-neutral-800/80' : 'border-neutral-200']">
          <Receipt class="h-4 w-4 text-neutral-400 shrink-0" />
          <span :class="['text-xs font-bold uppercase tracking-wider', isDark ? 'text-neutral-300' : 'text-neutral-600']">Recibo do Cliente</span>
        </div>

        <div class="space-y-2.5 text-xs text-neutral-400">
          <div class="flex items-center justify-between">
            <span>ID do Usuário:</span>
            <span :class="['font-mono font-semibold flex items-center gap-1', isDark ? 'text-neutral-200' : 'text-neutral-800']">
          ...
          <div :class="['flex items-center justify-between border-t border-dashed pt-2.5 mt-2.5', isDark ? 'border-neutral-800/80' : 'border-neutral-200']">
      ...
      <!-- Call To Action Button (Voltar para o Telegram) -->
      <a 
        :href="telegramBotUrl"
        target="_blank"
      ...
      <p :class="['text-[10px] mt-4 leading-normal', isDark ? 'text-neutral-500' : 'text-neutral-400']">
        Caso seu bot não responda imediatamente, envie o comando <code :class="['font-mono px-1 py-0.5 rounded border text-sky-500', isDark ? 'bg-neutral-950 border-neutral-800' : 'bg-neutral-100 border-neutral-200']">/start</code> nele para atualizar as credenciais.
      </p>
...
<style scoped>
/* Scoped keyframes for animations */
@keyframes pulseGlow {
  0%, 100% {
    transform: scale(1.1);
    opacity: 0.15;
